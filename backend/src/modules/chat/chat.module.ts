...
export async function buildExistingVectorStore(
  indexName: string,
): Promise<Chroma> {
...
export async function buildVectorStoreFromTexts(
  indexName: string,
  texts: string[],
  metadatas: object[],
): Promise<Chroma> {
  return await Chroma.fromTexts(texts, metadatas, new OpenAIEmbeddings(), {
    collectionName: indexName,
  });
}
  ...
  providers: [
    {
      ...
      useFactory: async () => {
        ...
        const collection: Collection<Session> =
